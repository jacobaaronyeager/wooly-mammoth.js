...
function DocumentConversion(options, comments) {
  ...
  var stylePreservations = preserverUtils.processOptions(options);
  ...
  function convertParagraphProperties(props, document) {
    for (const [key, prop] of Object.entries(props)) {
      if (document[key].hasOwnProperty('tag')) {
        if (prop.styleId) {
          document[key].tag.attributes.class = prop.styleId;
        }
        let cssStyles = [];
        if (stylePreservations.applyStyleIndents && prop.numbering) {

        }
        if (stylePreservations.applyStyleAlignment && prop.alignment) {
          cssStyles.push((prop.alignment != null) ? "text-align: " + prop.alignment + "; clear: both;" : undefined);
        }
        if (stylePreservations.applyStyleIndents && prop.indent) {
          const indentDivisor = 1440;
          cssStyles.push((prop.indent.start != null && prop.indent.start != 0) ? "margin-left: " + (prop.indent.start / indentDivisor) + "in;" : undefined);
          cssStyles.push((prop.indent.end != null && prop.indent.end != 0) ? "margin-right: " + (prop.indent.end / indentDivisor) + "in;" : undefined);
          cssStyles.push((prop.indent.firstLine != null && prop.indent.firstLine != 0) ? "text-indent: " + (prop.indent.firstLine / indentDivisor) + "in;" : undefined);
          cssStyles.push((prop.indent.hanging != null && prop.indent.hanging != 0) ? "text-indent: -" + (prop.indent.hanging / indentDivisor) + "in; padding-left: " + (prop.indent.hanging / indentDivisor) + "in;" : undefined);
        }
        if (stylePreservations.applyStyleColor && prop.color && prop.color !== "auto") {
          cssStyles.push((prop.color != null) ? 'color: #' + prop.color + " !important" : undefined);
        }
        cssStyles = _.compact( cssStyles );
        if (cssStyles.length > 0) {
          if (document[key].tag.attributes.style == undefined) {document[key].tag.attributes.style = '';}
          document[key].tag.attributes.style += cssStyles.join(' ');
        }
      }
    }
    return document;
  ...
  function convertRun(run, messages, options) {
    ...
    if (stylePreservations.useColorSpans && run.color && run.color !== "auto") {
      paths.push(htmlPaths.element('span', {style: 'color:#' + run.color + " !important"}, {fresh: false}));
    }
    if (stylePreservations.useFontSizeSpans && run.fontSize) {
      var fontSize = stylePreservations.useStrictFontSize ? Math.round(run.fontSize / 1.5) : run.fontSize;
      paths.push(htmlPaths.element('span', {style: 'font-size:' + fontSize + 'px'}, {fresh: false}));
    }
  ...
  function convertTable(element, messages, options) {
    var htmlPath = findHtmlPath(element, htmlPaths.element("table", {}, {fresh: true}));

    if (stylePreservations.applyTableStyles) {
      ...
      if (!stylePreservations.ignoreTableElementBorders) {
        if (htmlPath._elements && htmlPath._elements.length > 0) {
          if (element.preservedStyles) {
            htmlPath._elements[0].attributes.style = preserverUtils.convertPreservableStylesToCssString(element.preservedStyles, stylePreservations.reduceCellBorderStylesUsed);
          }
        }
      }
    }
  ...
  function convertTableCell(element, messages, options) {
    ...
    if (stylePreservations.applyTableStyles && element.preservedStyles) {
      ...
      attributes.style = preserverUtils.convertPreservableStylesToCssString(element.preservedStyles, stylePreservations.reduceCellBorderStylesUsed);
    }
  ...
  var elementConverters = {
    "document": function(document, messages, options) {
      ...
      children = convertParagraphProperties(document.children, children);
