...
function BodyReader(options) {
  ...
  function readParagraphIndent(element, style) {
    const elementIndent = {
    ...
    let styleIndent = {};
    if (preserverUtils.processOptions('applyStyleIndents')) {
      if (style.style != undefined && style.style.style != undefined) {
        const styleElements = style.style.style;
        for (let [k, v] of Object.entries(styleElements.children)) {
          for (let [key, value] of Object.entries(v.children)) {
            if (value.name == 'w:ind') {
              styleIndent = {
                start: value.attributes["w:start"] || value.attributes["w:left"],
                end: value.attributes["w:end"] || value.attributes["w:right"],
                firstLine: value.attributes["w:firstLine"],
                hanging: value.attributes["w:hanging"],
              }
            }
          }
        }
    ...
    return deepmerge(elementIndent, styleIndent);
  }

  function readAlignmentProperties(element, style) {
    if (preserverUtils.processOptions('applyStyleAlignment')) {
      if (style.style != undefined && style.style.style != undefined) {
        const styleElements = style.style.style;
        for (let [k, v] of Object.entries(styleElements.children)) {
          for (let [key, value] of Object.entries(v.children)) {
            if (value.name == "w:jc") {
              element = value.attributes['w:val'];
            }
          }
        }
      }
  ...
  function getGenericStyleElements(element, style, elementName, attributeSelector = 'w:val', perserverOption = 'anyOption') {
    let inlineElement = element.firstOrEmpty(elementName).attributes[attributeSelector] || null;

    if (preserverUtils.processOptions(perserverOption)) {
      if (style != undefined && style.style != undefined && style.style.style != undefined) {
        for (let [k, v] of Object.entries(style.style.style.children)) {
          for (let [key, value] of Object.entries(v.children)) {
            if (value.name == elementName) {
              return value.attributes[attributeSelector];
            }
          }
        }
      }
  ...
  function readStyle(element, styleTagName, styleType, findStyleById) {
    ...
    return elementResultWithMessages({styleId: styleId, name: name, style: style}, messages);
  }
  ...
  var xmlElementReaders = {
    ...
    "w:pPr": function(element) {
      return readStyle(element, "w:pStyle", "Paragraph", styles.findParagraphStyleById).map(function(style) {
        return {
          type: "paragraphProperties",
          styleId: style.styleId,
          styleName: style.name,
          alignment: readAlignmentProperties(element.firstOrEmpty("w:jc").attributes["w:val"], style),
          numbering: readNumberingProperties(element.firstOrEmpty("w:numPr"), numbering),
          indent: readParagraphIndent(element.firstOrEmpty("w:ind"), style),
          verticalAlignment: getGenericStyleElements(element, style, 'w:vertAlign'),
          color: getGenericStyleElements(element, style, 'w:color', 'w:val', 'applyStyleColor'),
        };
      });
    },
    ...
    "w:rPr": function (element) {
      return runStyles = readStyle(element, "w:rStyle", "Run", styles.findCharacterStyleById).map(function (style) {
        return {
          type: "runProperties",
          pStyleId: element.pStyle,
          styleId: style.styleId,
          styleName: style.name,
          verticalAlignment: element.firstOrEmpty("w:vertAlign").attributes["w:val"],
          font: element.firstOrEmpty("w:rFonts").attributes["w:ascii"],
          fontSize: element.firstOrEmpty('w:sz').attributes['w:val'],
  ...
  function readTable(element) {
    var tableProperties = element.firstOrEmpty("w:tblPr");
    var propertiesResult = readTableProperties(tableProperties);

    var preservableStyles = preserverUtils.extractPreservableTableStyles('table', tableProperties);

    return readXmlElements(element.children)
      .flatMap(calculateRowSpans)
      .flatMap(function(children) {
        return propertiesResult.map(function(properties) {
          properties.preservedStyles = preservableStyles;
  ...
  function readTableProperties(element) {
    return readStyle(element, "w:tblStyle", "Table", styles.findTableStyleById).map(function(style) {
      return {
        styleId: style.styleId,
        styleName: style.name
      };
  ...
  function readTableCell(element) {
    return readXmlElements(element.children).map(function(children) {
      ...
      var preservedStyles = preserverUtils.extractPreservableTableStyles('cell', properties);

      var gridSpan = properties.firstOrEmpty("w:gridSpan").attributes["w:val"];
      var colSpan = gridSpan ? parseInt(gridSpan, 10) : 1;

      var cell = documents.TableCell(children, {colSpan: colSpan, preservedStyles: preservedStyles});
